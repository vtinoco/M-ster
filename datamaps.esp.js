...
(function() {
  ...
  */
  ...
  function handleArcs (layer, data, options) {
	...
	//Manipulamos etiquetas de indices en concreto
	function handleLabels ( layer, options ) {
		var self = this;
		options = options || {};
		//Fijamos el centro de nuetro mapa, para centrar etiquetas, tiene que coincidir con el mapa. 
		var labelStartCoodinates = this.projection([13, 44]);
		this.svg.selectAll(".datamaps-subunit")
		  .attr("data-foo", function(d) {
			
			var center = self.path.centroid(d);
			
			var xOffset = 6, yOffset = 5;
			
			//Para ver bien Ceuta, Melilla y Pontevedra
			if ( d.id === "Me" ) yOffset = 18; 
			if ( d.id === "Ce" ) yOffset = 18;
			if ( d.id === "Po" ) xOffset = 8;
			
			//Una vez definido marcos vamos con el codigo que nos permita etiquetar 
			var x,y;

			x = center[0] - xOffset;
			y = center[1] + yOffset;
			
			//Con smallProvinceIndex decimos cuales son las que queremos etiquetar fuera con una dotterline. 
			var smallProvinceIndex = ["Bi", "SS", "Vi"].indexOf(d.id);
			if ( smallProvinceIndex > -1) {
			  var yStart = labelStartCoodinates[1] ;
			  x = labelStartCoodinates[0] - 300;
			  y = yStart + (smallProvinceIndex * (2 + (options.fontSize || 12)));
			  layer.append("line")
				.attr("x1", x - 15)
				.attr("y1", y - 5)
				.attr("x2", center[0])
			...
			//Definimos el tipo de etiqueta
			layer.append("text")
			  .attr("x", x)
			  .attr("y", y)
			  .style("font-size", (options.fontSize || 10) + 'px')
			  .style("font-family", options.fontFamily || "Verdana")
			  .style("font-weight", options.fontWeight || "bold")
